...
import ScheduleList from "./AppointmentList";

const AppointmentModal = ({ visible, onclose, setSelectTime, date }) => {
    const schedule = [
          ...
          <View style={styles.scheduleContainer}>
            <ScrollView contentContainerStyle={styles.scheduleContent} >
                {schedule.map((item, index) => (
                    <ScheduleCard key={index} time={item.time} isAvailable={item.isAvailable} setSelectTime={setSelectTime} onclose={onclose} />
                ))}
            </ScrollView>
          </View>
          <View>
  ...
  const ScheduleCard = ({ time, isAvailable, setSelectTime, onclose }) => {
